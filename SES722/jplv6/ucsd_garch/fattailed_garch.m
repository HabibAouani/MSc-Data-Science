...
% PURPOSE: FATTAILED_GARCH(P,Q) parameter estimation with different error distributions, the NOrmal, The T, 
...
%   mcc -V1.2 -ir garchlikelihood
%   as it will improve pervormance about 6 times
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
[parameters, LLF, EXITFLAG, OUTPUT, LAMBDA, GRAD, HESSIAN] =  fmincon('fattailed_garchlikelihood', startingvals ,sumA  , sumB ,[] , [] , LB , UB,[],options, data, p , q, errortype);
...
hess = hessian('fattailed_garchlikelihood',parameters,data,p,q,errortype);
[likelihood, ht]=fattailed_garchlikelihood(parameters,data,p,q,errortype);
...
if nargout > 4
   ...
   for i=1:length(parameters)
      ...
      [HOLDER, HOLDER1, indivlike] = fattailed_garchlikelihood(hparameters,data,p,q,errortype);
