...
case {'sar_gg'} % <=================== MCMC spatial autoregressive model Zellner g-prior
...
fprintf(fid,'\n');
fprintf(fid,'Bayesian spatial autoregressive model, Zellner g-prior \n');
if results.novi == 1
    fprintf(fid,'Homoscedastic version \n');
elseif results.novi == 0
    fprintf(fid,'Heteroscedastic model \n');
...
if results.dflag == 0
    fprintf(fid,'numerical integration used for rho \n');
elseif results.dflag == 1
...
case {'sar_gc'} % <=================== c-mex MCMC spatial autoregressive model
...
rsqr = results.rsqr; % conventional r-squared
...
fprintf(fid,'mean of sige draws = %9.4f \n',mean(results.sdraw));
if results.mflag == 1
fprintf(fid,'log marginal post  = %9.4f \n',results.mlike);
end;    
if results.rdraw == 0
fprintf(fid,'r-value            = %6d   \n',results.r);
else
...
fprintf(fid,'seed value         = %10d \n',results.seed);
fprintf(fid,'min and max rho    = %9.4f,%9.4f \n',results.rmin,results.rmax);
fprintf(fid,'***************************************************************\n');

if results.priorb == 1 % prior informative priors only
vstring = 'Variable';
bstring = 'Prior Mean';
tstring = 'Std Deviation';
